...
public class DataBaseInitializer {
    ...
    //USERS, DIRECTIONS & SHOPPING CARTS
        ...
        Optional<Direction> address01 = directionRepo.findByCode(address.getCode());
        ...
        if(!user01.isPresent()){
            userRepo.save(user1);
            User user1Redis = new User();
            user1Redis.setId(user1.getId());
            user1Redis.setUsername(user1.getUsername());
            user1Redis.setEmail(user1.getEmail());
            user1Redis.setRol(user1.getRol());
            user1Redis.setPassword(user1.getPassword());
            user1Redis.setPasswordConfirmation(user1.getPasswordConfirmation());
            user1Redis.setDirection(user1.getDirection());
            user1Redis.setImageFile(null);
            redisTemplate.opsForValue().set("user:administrator1", user1Redis);
        }
        ...
        Optional<Direction> address02 = directionRepo.findByCode(address2.getCode());
        ...
        if(!user02.isPresent()){
            ...
            userRepo.save(user2);
            User user2Redis = new User();
            user2Redis.setId(user2.getId());
            user2Redis.setUsername(user2.getUsername());
            user2Redis.setName(user2.getName());
            user2Redis.setEmail(user2.getEmail());
            user2Redis.setRol(user2.getRol());
            user2Redis.setPassword(user2.getPassword());
            user2Redis.setPasswordConfirmation(user2.getPasswordConfirmation());
            user2Redis.setDirection(user2.getDirection());
            user2Redis.setShoppingCart(user2.getShoppingCart());
            user2Redis.setImageFile(null);
            redisTemplate.opsForValue().set("user:carlos", user2Redis);
        }
        ...
        if(!user03.isPresent()){
            userRepo.save(user3);
            User user3Redis = new User();
            user3Redis.setId(user3.getId());
            user3Redis.setUsername(user3.getUsername());
            user3Redis.setName(user3.getName());
            user3Redis.setEmail(user3.getEmail());
            user3Redis.setImageFile(null);
            redisTemplate.opsForValue().set("user:anonymous", user3Redis);
        }
